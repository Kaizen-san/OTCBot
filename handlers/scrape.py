...
async def scrape_x_profile(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    twitter_url = ticker_data.get_twitter_url()
    
    try:
        tweets = await scrape_tweets(twitter_url)
        formatted_tweets = format_tweets(tweets, twitter_url, ticker)
        await query.edit_message_text(formatted_tweets, parse_mode='HTML', disable_web_page_preview=True)
