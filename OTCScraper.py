...
async def get_company_profile(session: aiohttp.ClientSession, ticker: str) -> Dict[str, Any]:
    url = f"{BASE_URL}/company/profile/full/{ticker}?symbol={ticker}"
    return await fetch_data(session, url)

async def get_trade_data(session: aiohttp.ClientSession, ticker: str) -> Dict[str, Any]:
    url = f"{BASE_URL}/stock/trade/inside/{ticker}?symbol={ticker}"
    return await fetch_data(session, url)
...
async def info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        try:
            parsed_profile = await get_company_profile(session, ticker)
            parsed_trade = await get_trade_data(session, ticker)
